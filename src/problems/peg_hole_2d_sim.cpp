...
void simulate(Simulator &simulator, Model &model, unsigned int num_trials, unsigned int max_steps,
              std::ofstream &file, unsigned int display, unsigned int particles)
{
    ...
    for (unsigned int i = 0; i < num_trials; i++)
    {
        ...
        simulator.simulate(init_state, max_steps, display, particles, i);
        ...
        // Print to file.
        file << "trial " << i << ", reward = " << simulator.getCumulativeReward() << std::endl;
        file << ", sensing time = " << simulator.getAverageActiveSensingTime()  << std::endl;
        file << ", taskaction time = " << simulator.getAvgTaskactionTime();
        file << ", goal reached = " << model.isGoal(simulator.getStates().back()) << std::endl;
        file << ", rtt time = " << simulator.getAvgRttTime() << std::endl;
        file << ", recovery time= " << simulator.getRecoveryTime() << std::endl;
        if (model.isGoal(simulator.getStates().back()))
        {
            ...
            total_rtt += simulator.getAvgRttTime();
        }
    }
    ...
    file << "average rtt time" << total_rtt / num_successes << std::endl;
}
...
int main(int argc, char** argv)
{
    ...
    strftime(buffer, 80, "%Y-%m-%d-%H:%M:%Slocal", time_info);
  ...
  // Simulator random_simulator(model, random_planner, &node_handle, sensing_interval);
   ...
   /* 
    ...
    simulate(random_simulator, model, num_trials, max_steps, file, verbosity, num_particles);
    ...
    simulate(state_entropy_simulator, model, num_trials, max_steps, file, verbosity, num_particles);
    ...
    */
    ...
    simulate(action_entropy_simulator, model, num_trials, max_steps, file, verbosity, num_particles);
