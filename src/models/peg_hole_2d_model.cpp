...
{
    ROS_INFO("local state0 in function is %f",state(0));
    ROS_INFO("local state1 in function is %f",state(1));
    ROS_INFO("local task action0 is %f",task_action(0));
    ROS_INFO("local task action1 is %f",task_action(1));
    ROS_INFO("local task action2 is %f",task_action(2));
}
    Eigen::VectorXd noiseless_next_state = getNextState(state, task_action);
if(globalcheckflag==1)
{
    ROS_INFO("local new noiseless_next_state0 is %f",noiseless_next_state(0));
    ROS_INFO("local new noiseless_next_state1 is %f",noiseless_next_state(1));
...
if(globalcheckflag&&nextflag==1)
{
	 ...
	 i0flagcheck = 0;
if(globalcheckflag==1){
ROS_INFO("local noise(0)  is %f",noise(0));
ROS_INFO("local noise(1)  is %f",noise(1));
}
